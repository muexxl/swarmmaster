...
class PacketBuffer(object):
    def __init__(self):
        ...
        self.last_checksum_packet = DataPacket(bytearray(10))
        self.packet_history = ""
    ...
    def add_packet(self, packet: DataPacket):
        ...
        if packet.type == coco.DATA:
            if packet.id % 2 == 0:
                ...
                char = "\\"
            self.packet_history += char
        else:
            self.packet_history += str(packet.id) +" "


        self.received_bytes_brutto += packet.len
    ...
    def process_packet_buffer(self):
        ...
        while (len(self.packet_buffer)):
            new_packet = self.packet_buffer.pop(
                0)  #get first element -fifo style
            if new_packet.type == coco.DATA:
                ...
                #baaaaaad error
                logger.error(
                    f"[packet_buffer] Unkown Packet id received :{new_packet.id} {new_packet.data}"
                )

    def add_to_availables(self, new_packet: DataPacket):
        # if new_packet.id==1:
        #     self.tmp_packet=new_packet
        #     return #DEBUG ONLY !!! KILL ALL PACKETS WIT ID 1!!

        if self.is_packet_in_availables(new_packet):
            if self.are_packets_identical(
                    new_packet, self.available_packets[new_packet.id]):
                ...
                self.lost_packets += 1
                self.reset()
        if new_packet.id < self.last_packet:
            #bad . we missed at least the checksum
            self.lost_packets += 1
            self.reset()
    ...
    def process_packet(self, packet: DataPacket):
        ...
        if packet.id == self.last_packet + 1:
            ...
            # logger.debug(f"[Packetbuffer] Adding data to streambuffer{packet.data}")
            # logger.debug(f"[Packetbuffer] Stream Buffer lenght is {len(self.stream_buffer)}")
            self.stream_buffer_lock.release()
    ...
    def handle_checksum_packet(self, checksum_packet: DataPacket):
        ...
        if self.are_packets_identical(checksum_packet,
                                      self.last_checksum_packet):
            self.double_packets += 1
            return

        self.last_checksum_packet = checksum_packet
        ...
        if packets == available_packets:
            ...
            if available_packets == 0:
                ...
                logger.error(
                    f"[PacketBuffer] previous checksum packet was lost")
                # self.stream_buffer_lock.acquire()
                # self.stream_buffer=bytearray()
                # self.stream_buffer_lock.release()
                self.reset()
            ...
            logger.error(
                f"[PacketBuffer] {lostPackets} out of {packets} are lost. restauration was not possible"
            )
    ...
    def restore_packet(self, checksum_packet: DataPacket):
        ...
        for i in range(checksum_packet.id):
            if i in self.available_packets.keys():
                checksum ^= int.from_bytes(self.available_packets[i].data,
                                           'little')
            else:
                missing_id = i
