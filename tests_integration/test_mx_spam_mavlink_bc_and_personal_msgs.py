...
DUMMY_MAVLINK_MSG3=b'\xfd\xaaDIES_KOENNTE_EINE_GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAANZ SCHOEN LANGE_MAVLINK_V2_MSG_SEIN'
DUMMY_MAVLINK_MSG4=b'\xfd\xaaDIES_KOENNTE_EINE_GBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBNZ SCHOEN LBNGE_MBVLINK_V2_MSG_SEIN'
...
def run():
    ...
    while(1):
        ...
        #msg = None #Block registration requests
        if msg:
            sm.messagehandler.handle_msg(msg)

        sm.swarmmanager.broadcast_client.add_data_to_tx_buffer(DUMMY_MAVLINK_MSG4+f"{counter_bc:08d}\r\n".encode('ascii'))
        counter_bc +=1
        sm.broadcast_data()
        ...
        if client:
            ...
            while (len(client.tx_buffer)<100):
                client.add_data_to_tx_buffer(DUMMY_MAVLINK_MSG3+f"{counter:08d}\r\n".encode('ascii'))
                counter+=1
